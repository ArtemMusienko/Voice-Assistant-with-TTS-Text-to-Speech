...
Также в коде используется **JavaScript**, он нужен для того, чтобы в браузере (в **Google Colab**) записать звук с микрофона пользователя и вернуть его в **Python**-код в виде **base64**-строки. Соответственно, для работы с этим кодом потребуется предоставить доступ к микрофону.
